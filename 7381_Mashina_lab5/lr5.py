...
def singleModelPlots( kernel, history ):
    ...
    plt.title('Accuracy with '+str(kernel)+"x"+str(kernel)+' kernel size')
    plt.ylabel('Accuracy')
    plt.xlabel('Epoch')
    ...
    plt.title('Loss with '+str(kernel)+"x"+str(kernel)+' kernel size')
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.legend(['Train', 'Test'], loc='upper left')
    plt.show()
...
def buildModel(kernel_size=3):
    ...
    out = Dense(num_classes, activation='softmax')(drop_3)  # (hidden)#(drop_3)
